...
def mostrar_tabla(ciclistas):
    ...
    for ciclista in sorted(ciclistas, key=lambda x: x['tiempo']):  # Ordena por tiempo
        print(f"Código: {ciclista['codigo']}, Nombre: {ciclista['nombre']}, Edad: {ciclista['edad']}, País: {ciclista['pais']}, Equipo: {ciclista['equipo']}, Tiempo: {ciclista['tiempo']} min")

def corregir_tiempo(ciclistas, codigo, nuevo_tiempo):
    for i in range(len(ciclistas)):
        if ciclistas[i]['codigo'] == codigo:
            ciclistas[i]['tiempo'] = nuevo_tiempo
